...
Tài liệu mô tả **trải nghiệm người chơi** từ lúc vào server tới endgame, dựa trên hệ thống đã có. Không thay thế plan kỹ thuật [`prison-rpg-plan.md`](../archive/prison-rpg-plan.md) — file đó là roadmap triển khai plugin/config; file này là nguồn “người chơi làm gì / vì sao”.

Cập nhật: 2026-08-05 (Bang Hội soft-launch + KOTH/PvP bang; **Bang Chiến đang phát triển**; sell multiplier chỉ qua gang).

Mục lục: [`INDEX.md`](../INDEX.md). Hướng dẫn chơi bang: [`gangs.md`](gangs.md). Config/ops: [`plugins/CorePlugin/gang/README.md`](../README.md).
...
Nguồn hiện tại: `plugins/CorePlugin/prison/` và cấu hình rank trong `plugins/X-Prison/` (Skript `level_gate.sk` cũ đã được loại bỏ).
...
- Tham chiếu: [`npc-summoning-badge.md`](../admin/npc-summoning-badge.md), Skript `plugins/Skript/scripts/demon_king/`.
...
- Chuyển Sinh: [`systems/rebirth-attributes.md`](../systems/rebirth-attributes.md), nguồn sống `plugins/CorePlugin/rebirth/`
...
| **Bang Hội (CorePlugin)**   | `/gang` — tạo/join, bank, vault, upgrade, shop, quest bang, KOTH, home, season    | `plugins/CorePlugin/gang/` · [README](../README.md)                            |
| DailyQuest (CorePlugin)     | Daily/weekly **cá nhân** + check-in — **tách** quest bang                         | `plugins/CorePlugin/dailyquest/` · [`quests.md`](quests.md)                        |
...
| MMOCore                     | Class, level, skill; tích hợp gate rankup                                         | `plugins/MMOCore/classes/`, `plugins/CorePlugin/prison/`                                               |
| MythicMobs dungeon          | Farm combat / boss theo rank                                                      | `mobs/prison_rank_mobs.yml`, `docs/reference/spawners.md`                                                    |
...
| Jewelry / Long Tộc / crates | Bonus / sink phụ — **Thiên Giới** = paid giáp/VK; Long Tộc / Free / Ngọc xem plan | `docs/systems/crate-thien-gioi.md`, `docs/archive/crates-refactor.md`, ExcellentCrates                      |

**Không dùng** X-Prison module `gangs.yml` cho người chơi — lối chơi bang = **CorePlugin gang** thôi.

Quy chuẩn chỉ số combat (không phải journey): [`STAT_BALANCING_STANDARD.md`](../../STAT_BALANCING_STANDARD.md), [`stat-abbreviations.md`](../reference/stat-abbreviations.md).
...
| --: | -------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
|   A | **DailyQuest cá nhân**           | Login → điểm danh / đào / giết → claim; weekly clear-all → chìa Long Tộc | **Đã có** CorePlugin DailyQuest · [`quests.md`](quests.md). BattlePass daily = thử nghiệm cũ, không bắt buộc      | Soft-launch                  |
...
|   C | **Đêm Nguyệt Huyết**             | Cuối tuần 2 giờ: boss Bá Tước Huyết Nguyệt                               | **Đã làm** — Skript `bloodmoon/` · [`nguyet-huyet-notes.md`](../systems/blood-moon-event.md)                                                   | P1 xong                      |
...
|   H | **Chìa khóa tuần từ daily**      | Weekly clear-all DailyQuest → chìa Long Tộc (đã config)                  | ExcellentCrates + DailyQuest weekly — xem `quests.md`                                                                       | Soft-launch (một phần A)     |
...
|   6 | **Cân bằng kinh tế Phase 6**       | ~~Ore/EV lệch late + prestige cliff~~                           | **Đã chỉnh 2026-07-29** — chi tiết [`economy-phase6.md`](../archive/economy-phase6.md); reload `/sreload` + `/xprison reload` + `sk reload prison/`                                          |
...
|   9 | **DailyQuest + Bang Hội**          | Login ngắn (A) + xã hội/KOTH (A2–A3)                            | **Soft-launch.** DailyQuest + gang đã có; ops: region KOTH WG, `/gang reload`, test create/vault/quest/KOTH. BattlePass daily = cũ ([`battlepass-daily.md`](../archive/battlepass-daily.md)) |
...
|  11 | **Đêm Nguyệt Huyết**               | Sự kiện cuối tuần (§6.2 C)                                      | **Đã làm 2026-07-29** — [`nguyet-huyet-notes.md`](../systems/blood-moon-event.md); nhớ `/nguyethuyet setspawn`                                                                                         |
