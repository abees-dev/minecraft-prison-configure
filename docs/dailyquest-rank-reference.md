...
> Nguồn: `docs/xprison-config-notes.md` §7 (mine composition), `plugins/CorePlugin/prison/config.yml`, `plugins/MythicMobs/mobs/prison_rank_mobs.yml`, `plugins/MMOItems/crafting-stations/refinery-*.yml`, `docs/SPAWNER_LIST.md`.
>
...
Composition live từ `docs/xprison-config-notes.md` §7. Dùng làm `BREAK_BLOCK` `filter:` + `worlds: [world_prison]`.
...
- Ore filter **phải** nằm trong composition mine tương ứng (`xprison-config-notes.md` §7).
