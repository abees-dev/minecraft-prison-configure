...
| Hologram | `DecentHolograms/holograms/MaVuongTimer.yml` | Dòng 1 tĩnh "Bàn Thờ Triệu Hồi Ma Vương". Dòng 2 **tự động cập nhật mỗi 5 giây** bởi Skript (`every 5 seconds` trong `mavuong_altar.sk`, dùng lệnh `/dh line set MaVuongTimer 1 2 <text>`) — hiện "Sẵn sàng - Click để triệu hồi!" hoặc "Có thể triệu hồi lại sau: X phút Y giây". Chưa export qua PlaceholderAPI vì server không có addon Skript-PAPI (SkBee, skript-placeholders...) — cách đẩy trực tiếp qua `/dh line set` hiện tại vẫn nhẹ, không cần thêm plugin. |

- Rớt 25% từ boss trùm cuối: `TINHTHE_HUY_DIET` ("Tinh Thể Hỗn Mang - Hủy Diệt", item MMOItems MATERIAL có sẵn nhưng trước đây chưa gắn vào đâu) — dùng để rèn/trade trang bị cao cấp.
- **Chuỗi vật phẩm triệu hồi (đã đổi)**: 9 boss rank 1–9 rớt 100% `MANH_HUY_HIEU_TRIEU_HOI` ("Huy Hiệu Vỡ", MMOItems MATERIAL gốc PRISMARINE_SHARD) thay vì huy hiệu thật trực tiếp như trước → **cần chế (craft) mảnh vỡ thành `HUY_HIEU_TRIEU_HOI`** (huy hiệu thật, gốc NETHER_STAR) → chỉ cần **1 huy hiệu thật** trong túi để click bàn thờ triệu hồi. **Công thức chế mảnh vỡ → huy hiệu thật CHƯA làm** (theo yêu cầu "chế sau") — hiện chưa có cách nào lấy `HUY_HIEU_TRIEU_HOI` trong game, cần làm tiếp phần này khi bạn sẵn sàng chốt tỉ lệ/cách chế.
