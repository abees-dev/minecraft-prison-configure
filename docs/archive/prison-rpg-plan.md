# Kế hoạch server Minecraft RPG tích hợp Prison

> **Lối chơi người chơi (journey + gap roadmap + lớp chống nhàm):** xem [`gameplay.md`](../player/gameplay.md). Mục lục toàn bộ docs: [`INDEX.md`](../INDEX.md). File này giữ vai trò roadmap kỹ thuật / phase triển khai plugin.

## Context

Server Paper 1.19.4-550 tại `<server-root>` hiện đã cài rất nhiều plugin RPG/tiện ích (MMOCore, MMOItems, MythicMobs, ModelEngine, ItemsAdder, RPGInventory, LuckPerms, Vault, EssentialsX, Multiverse-Core/Inventories, WorldGuard+FastAsyncWorldEdit, PlayerPoints, ExcellentCrates, PlaceholderAPI, DeluxeMenus, TAB, DecentHolograms, Vulcan, Skript, SCore/nightcore, BattlePass, Citizens, LibsDisguises, PlayerVaults, v.v.) nhưng qua khảo sát trực tiếp, **gần như toàn bộ đang ở trạng thái mặc định**:
...
**Icon Đá Nâng Cấp đổi theo block của mỏ + thêm glow (2026-07-21)**: icon cũ dùng chung `AMETHYST_SHARD` cho cả 9 rank — đổi thành đúng vật liệu của mỏ đó. Thêm hiệu ứng lấp lánh (enchant glow) để phân biệt với block thường trong túi đồ — dùng thủ thuật MMOItems tiêu chuẩn: `enchants: {unbreaking: 1}` + `hide-enchants: true` (enchant không có tác dụng thật vì item là MATERIAL không dùng được, chỉ để lấy hiệu ứng glow, không hiện chữ enchant trong lore).

Lần đầu dùng ORE block (`COAL_ORE`, `COPPER_ORE`...) — sau đó người dùng chỉnh lại: phải dùng **block đã nén/craft** (vd Coal → `COAL_BLOCK`, không phải `COAL_ORE`), vì đây là vật phẩm đã "chế" ra chứ không phải quặng thô. Bảng icon cuối cùng: Tân Binh=`COBBLESTONE` (không có dạng nén khác nên giữ nguyên), Tù Nhân=`COAL_BLOCK`, Lao Công=`COPPER_BLOCK`, Thợ Đào=`IRON_BLOCK`, Đội Trưởng=`GOLD_BLOCK`, Phó Quản Ngục=`REDSTONE_BLOCK`, Quản Ngục=`LAPIS_BLOCK`, Bá Chủ Ngục Tù=`DIAMOND_BLOCK`, Vượt Ngục=`EMERALD_BLOCK`. Đã `mi reload all` + test give item cho player, không lỗi.

**Recipe "chế đá" đổi nguyên liệu sang block + bỏ thời gian chờ, nguyên liệu nâng cấp cuốc tăng theo cấp số nhân (2026-07-21)**: theo yêu cầu người dùng, 2 thay đổi trong cả 9 file `forge-*.yml` (lưu ý: 9 file này đã được người dùng tự đổi tên từ `forge-a..i.yml` sang `forge-1..9.yml` bên ngoài phiên làm việc — nội dung/mapping rank không đổi, forge-1=Tân Binh...forge-9=Vượt Ngục):
- **Ingredient recipe "chế đá" đổi từ quặng thô sang block đã nén** (bắt buộc player tự vanilla-craft 9 quặng thành 1 block trước khi đem chế): Tân Binh vẫn `COBBLESTONE` (không có dạng nén khác, tăng số lượng 16→24 để giữ độ khó tương đương) — 8 rank còn lại đổi thành block tương ứng, số lượng 2 block/lần chế: Tù Nhân=`COAL_BLOCK`, Lao Công=`COPPER_BLOCK`, Thợ Đào=`IRON_BLOCK`, Đội Trưởng=`GOLD_BLOCK`, Phó Quản Ngục=`REDSTONE_BLOCK`, Quản Ngục=`LAPIS_BLOCK`, Bá Chủ Ngục Tù=`DIAMOND_BLOCK`, Vượt Ngục=`EMERALD_BLOCK`.
- **Bỏ `crafting-time`** của recipe "chế đá" (3 giây → 0) — độ khó giờ nằm ở việc phải tự craft block trước, không cần thêm thời gian chờ.
- **Nguyên liệu Đá Nâng Cấp cho mỗi bước nâng cấp cuốc đổi sang cấp số nhân (×2 mỗi bước), tăng cao hơn hẳn**: cũ 2/3/4/6 (I→II/II→III/III→IV/IV→V) → mới **4/8/16/32**.
- ⚠️ **Bài học kỹ thuật**: script sửa hàng loạt ban đầu dùng chuỗi `replace` tuần tự theo từng cặp giá trị cũ→mới (2→4, 3→8, 4→16, 6→32) — bị lỗi double-substitution vì giá trị "2" sau khi đổi thành "4" ở bước 1 lại bị bước 3 (tìm "4"→"16") bắt nhầm, khiến bước-1 của cả 9 file bị sai thành 16 thay vì 4. Đã phát hiện qua đọc lại file sau khi chạy script (không tin tưởng mù quáng vào "chạy xong không lỗi") và viết script sửa riêng dựa theo **vị trí recipe** (tên recipe kết thúc bằng `-1-2`) thay vì theo giá trị, rồi verify lại toàn bộ 9 file bằng `grep` trước khi reload. Bài học: khi sửa hàng loạt bằng chuỗi thay thế cascading trên cùng 1 tập giá trị số, luôn kiểm tra lại kết quả cuối, đừng giả định script chạy xong là đúng.
- Đã `mi reload all` — vẫn 12 Crafting Stations / 45 Recipes, không lỗi mới liên quan `forge-*`.
