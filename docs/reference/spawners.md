...
> Lối chơi / gap spawner: [`gameplay.md`](../player/gameplay.md) P0.2 · Hướng dẫn set vị trí: [`spawner-positions.md`](../admin/spawner-positions.md) · Mục lục: [`INDEX.md`](../INDEX.md)
...
> `spawner-positions.md` để set. Chỉ rank Tân Binh (Normal/Elite/Boss) đã có
> toạ độ thật.
...
- **Drop Ma Vương** (`prison_ma_vuong_boss_drops`): `TINHTHE_HUY_DIET` 25%, `DA_CUONG_HOA_THIEN_MENH` 15%, đá cường hóa Huyền Thoại 20% mỗi loại, chìa trang sức free 15%, chìa ngọc 50% (skill). Sink: đổi `TINHTHE_HUY_DIET` tại station `huy-hieu-trieu-hoi` → 2× Đá Thiên Mệnh hoặc 1× Đá Đục Lỗ Thiên Mệnh.
- **Chuỗi triệu hồi:** Boss rank 1–9 rớt `MANH_HUY_HIEU_TRIEU_HOI` (30%–50% theo rank) → chế tại `huy-hieu-trieu-hoi` (25 mảnh + $2.5M) → `HUY_HIEU_TRIEU_HOI` → click bàn thờ. Chi tiết drop: [`HUONG_DAN_DROPS_VA_EXP_NHATU.md`](prison-drops-exp.md).
